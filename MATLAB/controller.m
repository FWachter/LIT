...
% DATE CREATED: 2016-03-12 | LAST MODIFIED: 2016-04-07
...
classdef controller < handle
    ...
    methods
        ...
        % TYPE: Public Method
        % COMMENTS: Simulation Function
        % UPDATES: Added in acceleration function using MATLAB diff
            ...
            % arduinoObject.simulation.acceleration = @(time)((linearity.*totalTime.*(1 - time./totalTime).^linearity.*(endpoint - initial).*(time./totalTime).^linearity.*(2.*time.*(time./totalTime).^linearity - totalTime.*(time./totalTime).^linearity + 2.*time.*(1 - time./totalTime).^linearity - totalTime.*(1 - time./totalTime).^linearity + linearity.*totalTime.*(1 - time./totalTime).^linearity - linearity.*totalTime.*(time./totalTime).^linearity))/(time.^2.*((time./totalTime).^linearity + (1 - time./totalTime).^linearity).^3.*(time - totalTime).^2));
